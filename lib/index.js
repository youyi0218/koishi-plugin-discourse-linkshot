const { Context, h, Logger, Middleware, Schema, Session } = require('koishi')
const TEXT = {
...
function extractUrls(content) {
    ...
    return [
        ...new Set(matches.map(trimUrlCandidate).filter(Boolean).filter((value)=>{
            try {
                new URL(value);
                return true;
            } catch  {
...
function createDiscourseCookiesForOrigins(cookieSource, origins) {
    ...
    const normalizedOrigins = [
        ...new Set(origins.map((origin)=>normalizeOrigin(origin)).filter((origin)=>!!origin))
    ];
    return normalizedOrigins.flatMap((origin)=>cookiePairs.map(({ name, value })=>({
                name,
                value,
                url: origin
            })));
...
function createDiscourseCookies(cookieSource, forumOrigin) {
    return createDiscourseCookiesForOrigins(cookieSource, [
        forumOrigin
    ]);
}
...
function formatCaptureFailureMessage(targetUrl, publicError, authError) {
    const lines = [
        `${TEXT.failureMessage}${targetUrl}`,
        `${TEXT.publicErrorLabel}\uff1a${summarizeAttemptError(publicError)}`
    ];
...
function createSiteJsonUrl(config) {
...
async function sendSnapshot(session, buffer) {
    ...
    await session.send(h('img', {
        src
    }));
}
function normalizeForwardItemUrl(value) {
    const source = value?.trim();
    if (!source) return '';
    try {
...
function inferForwardItemTitle(url, fallback) {
...
function dedupeForwardItems(items) {
    const map = new Map();
...
function normalizeCaptureResult(result) {
    ...
    if (result && typeof result === 'object' && Buffer.isBuffer(result.buffer)) {
        return {
            ...
            forwardItems: dedupeForwardItems(result.forwardItems)
        };
    }
    throw new Error('\u622a\u56fe\u6e32\u67d3\u5668\u672a\u8fd4\u56de\u6709\u6548\u7684\u56fe\u7247\u6570\u636e\u3002');
}
function toNapCatActionName(action) {
    return action.replace(/_([a-z])/g, (_, letter)=>letter.toUpperCase());
}
function isDirectSession(session) {
    const directSession = session;
    return directSession.isDirect === true || directSession.subtype === 'private' || directSession.channel?.type === 1 || !session.guildId && !!session.userId && session.channelId === session.userId;
}
function resolveNapCatForwardTarget(session) {
    if (isDirectSession(session) && session.userId) return {
        direct: true,
...
async function callNapCatAction(session, action, params) {
    const bot = session.bot;
    const internal = bot?.internal;
    const actionNames = [
        action,
        toNapCatActionName(action)
    ];
    for (const target of [
        internal,
        bot
...
async function sendNapCatMergedForward(session, items) {
    ...
    const senderId = String(session.bot?.selfId || session?.selfId || session.userId || 0);
...
function selectCommentPosts(posts, requestedPostNumber, commentWindowCount) {
...
function renderCommentCard(post, baseOrigin) {
...
function renderCommentSections(posts, baseOrigin) {
...
function renderTopicHtml(payload, opPost, commentPosts, baseOrigin, sitePayload) {
...
async function extractTopicFromPage(page, url, requestedPostNumber, config) {
    ...
    const snapshot = await page.evaluate(({ requestedPostNumber, script })=>new Function('requestedPostNumber', script)(requestedPostNumber), {
        requestedPostNumber,
        script: DISCOURSE_DOM_EXTRACT_SCRIPT
    });
...
function toTopicPosts(posts) {
    return (posts || []).map((post)=>toTopicPost(post)).filter((post)=>!!post);
}
function mergeExtractedPosts(...postGroups) {
...
function buildRenderDataFromExtractedTopic(snapshot) {
...
function createBrowserLaunchOptions(config, useProxy = true) {
    ...
    if (useProxy && config.proxyServer) {
        options.proxy = {
            ...
            bypass: config.proxyBypass || undefined
        };
    }
...
function createLinkshotMiddleware(config, renderer, logger) {
    return async (session, next)=>{
        ...
        const captureWithProxyFallback = async (authenticated)=>{
            if (!config.proxyServer) return renderer.capture(targetUrl, {
                authenticated,
                useProxy: false
            });
            try {
                return await renderer.capture(targetUrl, {
                    authenticated,
                    useProxy: true
                });
            } catch (proxyError) {
...
class PlaywrightDiscourseRenderer {
    config;
    proxyBrowserTask;
    directBrowserTask;
    constructor(config){
        ...
        try {
            ...
            return this.withPage(browser, baseOrigin, authenticated, async (page)=>{
                await page.setContent(html, {
                    waitUntil: this.config.pageWaitUntil,
                    timeout: this.config.navigationTimeout
                });
                await page.evaluate((script)=>new Function(script)(), SNAPSHOT_POST_PROCESS_SCRIPT);
                const forwardItems = await page.evaluate((script)=>new Function(script)(), SNAPSHOT_FORWARD_ITEM_SCRIPT);
