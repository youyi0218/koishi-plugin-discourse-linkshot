...
__export(index_exports, {
  ...
  selectCommentPosts: () => selectCommentPosts,
...
var Config = import_koishi.Schema.object({
  ...
  pageWaitUntil: import_koishi.Schema.union([
    import_koishi.Schema.const("domcontentloaded").description("推荐：页面主结构就绪后继续抓取。"),
    import_koishi.Schema.const("load").description("等待 load 事件；若超时建议改用 domcontentloaded。"),
    import_koishi.Schema.const("networkidle").description("等待网络空闲；适合静态页面，不适合长连接站点。")
  ]).role("radio").default("domcontentloaded").description(TEXT.pageWaitUntil),
  browserTimeout: import_koishi.Schema.number().description(TEXT.browserTimeout).default(DEFAULT_TIMEOUT),
  captureDelay: import_koishi.Schema.number().description(TEXT.captureDelay).default(DEFAULT_DELAY),
  commentWindowCount: import_koishi.Schema.natural().min(1).description(TEXT.commentWindowCount),
  ...
  closeBrowserAfterCapture: import_koishi.Schema.boolean().description(TEXT.closeBrowserAfterCapture).default(false),
...
__name(sortPostsByNumber, "sortPostsByNumber");
...
__name(selectCommentPosts, "selectCommentPosts");
...
__name(renderCommentCard, "renderCommentCard");
...
__name(renderCommentSections, "renderCommentSections");
...
__name(toTopicPosts, "toTopicPosts");
...
__name(mergeExtractedPosts, "mergeExtractedPosts");
...
var PlaywrightDiscourseRenderer = class {
  ...
  memoizeBrowserTask(useProxy) {
    const task = this.launch(useProxy).catch((error) => {
      if (useProxy) this.proxyBrowserTask = void 0;
      else this.directBrowserTask = void 0;
      throw error;
    });
  ...
  async closeBrowser(useProxy) {
    ...
    if (useProxy && this.config.proxyServer) this.proxyBrowserTask = void 0;
    else this.directBrowserTask = void 0;
